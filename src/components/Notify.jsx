...
const getLoggedInUserData = () => {
    try {
        ...
        return { roll_no: null, name: 'Unknown', last_name: '' };
    }
}
...
// ----------------------------------------------------------------------
// 2. InteractionsModal Component (Likes/Comments Viewer)
// ----------------------------------------------------------------------
const InteractionsModal = ({ interactionState, onClose, profileImageUrls, memberDetails }) => {
    if (!interactionState || !interactionState.post) return null;
    
    const { post, initialTab } = interactionState;
    // FIX: Default or initial tab now prefers 'likes'
    ...
    const renderLikesTab = () => (
        <div className="flex-grow overflow-y-auto p-4 space-y-3 pb-24">
            {likes.length === 0 ? (
                <p className="text-center text-gray-500 mt-5 text-sm">Be the first to like this post!</p>
            ) : (
                likes.map((rollNo, index) => { 
                    const member = memberDetails[rollNo]; 
                    const fullName = member ? `${member.name} ${member.last_name}`.trim() : `Loading...`;
                    const isPlaceholder = !member;

                    const profileImageUrl = profileImageUrls[rollNo];
                    const hasProfileImage = profileImageUrl !== null && profileImageUrl !== undefined;

                    return (
                        <div key={rollNo || index} className="flex items-start mt-1 p-2 rounded-lg transition-colors hover:bg-blue-50/50">
                            {/* Profile Image/Icon */}
                            {hasProfileImage ? (
                                <img
                                    src={profileImageUrl}
                            ...
                            <div className="text-sm flex-grow">
                                <p className={`font-semibold ${isPlaceholder ? 'text-gray-500 italic' : 'text-gray-800'}`}>
    ...
    const renderCommentsTab = () => (
        <div className="flex-grow overflow-y-auto p-4 space-y-4 pb-24">
            {sortedComments.length === 0 ? (
                <p className="text-center text-gray-500 mt-10 text-sm">No comments yet. Start the conversation!</p>
            ) : (
    ...
    // Tab styles
    const tabClass = "flex-1 text-center py-3 text-sm font-bold transition-colors";
    const activeTabClass = "text-blue-600 border-b-2 border-blue-600";
    const inactiveTabClass = "text-gray-500 hover:bg-gray-100 border-b-2 border-transparent";
            ...
            <div className="bg-white w-full h-[90%] max-w-lg rounded-t-3xl shadow-2xl flex flex-col">
                ...
                <div className="flex border-b border-gray-200">
                    <button 
                        ...
                        className={`${tabClass} ${activeTab === 'likes' ? activeTabClass : inactiveTabClass}`}
                    >
                        Likes ({likes.length})
                    </button>
                    <button 
                        onClick={() => setActiveTab('comments')} 
                        className={`${tabClass} ${activeTab === 'comments' ? activeTabClass : inactiveTabClass}`}
                    >
...
// ----------------------------------------------------------------------
const PostActions = ({ post, currentRollNo, onLikeToggle, onCommentClick, onLikesClick }) => {
    ...
    return (
        <div className="px-4">
            ...
            <div className="flex justify-between items-center text-xs text-gray-500 py-3 border-b border-gray-100">
                ...
                {commentCount > 0 && (
                    <button onClick={() => onCommentClick(post)} className="cursor-pointer font-medium text-gray-600 hover:text-gray-800 transition-colors">
            ...
            <div className="flex justify-around items-center py-2">
                ...
                <button
                    ...
                    className={`flex items-center justify-center w-1/2 gap-2 p-2 rounded-full transition-all duration-200 text-sm font-semibold ${
                        hasLiked ? "text-blue-600 bg-blue-50" : "text-gray-500 hover:bg-gray-100"
                    }`}
                >
...
const CommentInputField = ({ postId, userData, onCommentSubmit }) => {
    ...
    return (
        <div className="px-4 pb-3 pt-1 border-t border-gray-100">
            ...
            {!rollNo && (
                <p className="text-xs text-red-500 mt-1 text-center">You must be logged in to comment.</p>
            )}
        </div>
    );
};
...
const Notify = () => {
  ...
  const openCommentsModal = useCallback((post) => {
  ...
  const handleLikeToggle = useCallback(async (postId, currentlyLiked) => {
    const postRef = doc(db, "post", postId);
    if (!loggedInRollNo) return; 
    
    const action = currentlyLiked ? arrayRemove(loggedInRollNo) : arrayUnion(loggedInRollNo);
    ...
    try {
      ...
      if (interactionState && interactionState.post.id === postId) {
          fetchLikedMemberDetails(interactionState.post.comments.map(c => c.roll_no).concat(newLikes));
      }
  ...
  useEffect(() => {
    const fetchAndProcessPosts = async () => {
        try {
          ...
          const allRollNumbers = new Set(postsToProcess.flatMap(post => [
            post.roll_number, 
            ...(post.likes || []), 
            ...(post.comments || []).map(c => c.roll_no)
          ].filter(Boolean)));

          const profileUrlPromises = Array.from(allRollNumbers).map(rollNo => getProfileImageUrl(rollNo));
      ...
      <div className="flex flex-col gap-5 py-1 font-[Inter,sans-serif]">
        {posts.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center p-6 mt-12">
            <MdNotificationsNone
              className="text-8xl mb-4 text-gray-300"
            />
            <p
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-gray-900 flex items-center">
                        ...
                        {post.roll_number === "ADMIN_ROLL" && <MdVerified className="text-blue-500 ml-1 text-xs" />}
                      </p>
