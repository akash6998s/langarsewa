...
import { MdDeleteForever, MdNotificationsNone, MdAddCircle, MdThumbUp, MdComment, MdSend, MdClose, MdOutlineAccessTime, MdVerified, MdMoreVert, MdRssFeed } from "react-icons/md";
...
const getLoggedUser = () => { 
    try { 
        const m = JSON.parse(localStorage.getItem("loggedInMember")); 
        return { 
            roll_no: m?.roll_no || null, 
            name: m?.name || "Unknown", 
        ...
        console.log(e)
        return { roll_no: null, name: "Unknown", last_name: "", isSuperAdmin: false, isAdmin: false }; 
    } 
};
...
const formatTs = (ts) => { 
    ...
    try { 
        return (ts instanceof Timestamp ? ts.toDate() : new Date(ts)).toLocaleString("en-US", { 
...
const ProfilePic = (rollNo, name, lastName, url) => url ? ( 
    <img src={url} alt={name} className="w-10 h-10 rounded-full object-cover border-2 border-blue-200 flex-shrink-0 mr-3 mt-0.5" /> 
) : ( 
...
const MenuDropdown = ({ postId, showDeleteButton, onDeletePost, isComment, comment, loggedInRollNo, isAdmin, onDeleteComment }) => {
    ...
    const handleDelete = () => { 
        isComment ? onDeleteComment(postId, comment) : onDeletePost(postId); 
        setIsOpen(false); 
    };

    useEffect(() => { 
        const h = (e) => dropdownRef.current && !dropdownRef.current.contains(e.target) && setIsOpen(false); 
...
const InteractionsModal = ({ interactionState, onClose, profileImageUrls, memberDetails, onDeleteComment, loggedInRollNo, isAdmin }) => {
    ...
    const comments = [...(post.comments || [])].sort((a, b) => 
        (b.timestamp instanceof Timestamp ? b.timestamp.toMillis() : new Date(b.timestamp).getTime()) - 
        (a.timestamp instanceof Timestamp ? a.timestamp.toMillis() : new Date(a.timestamp).getTime())
    );
    ...
    const renderComments = () => (comments.length === 0 ? (<p className="text-center text-gray-500 mt-10 text-sm">No comments yet.</p>) : (
        comments.map((c, i) => (
    ...
    return (
        <div className="fixed inset-0 bg-gray-100 bg-opacity-80 z-50 flex items-end justify-center backdrop-blur-sm">
            <div className="bg-white w-full h-[90%] max-w-lg rounded-t-3xl shadow-2xl flex flex-col">
                <div className="flex justify-between items-center p-3 border-b border-gray-100">
                    <h3 className="text-lg font-bold text-gray-800">{activeTab === "comments" ? "Comments" : "Likes"}</h3>
...
const Notify = () => {
    ...
    const updatePostState = useCallback((postId, updateFn) => {
        setPosts(c => c.map(p => { 
            if (p.id === postId) { 
                const u = updateFn(p); 
                if(interactionState?.post.id === postId) setInteractionState(prev => ({ ...prev, post: u })); 
                return u; 
            } 
            return p; 
        }));
    ...
    const handleCommentSubmit = useCallback(async (postId, newComment) => {
        ...
        try { 
            ...
            updatePostState(postId, p => ({ ...p, comments: [...(p.comments || []), newComment] })); 
            if (!likedMemberDetails[newComment.roll_no]) fetchLikedDetails([newComment.roll_no]); 
    ...
    const handleDeleteComment = useCallback(async (postId, c) => {
        ...
        try { 
            ...
            updatePostState(postId, p => ({ ...p, comments: (p.comments || []).filter(cmt => cmt.roll_no !== c.roll_no || cmt.text !== c.text) })); 
        } catch (err) {console.log(err)
    ...
    const handleLikeToggle = useCallback(async (postId, liked) => {
        ...
        try { 
            await updateDoc(ref, { likes: action }); 
            updatePostState(postId, p => { 
                const newLikes = liked ? (p.likes || []).filter(r => r !== loggedInRollNo) : [...(p.likes || []), loggedInRollNo]; 
                !liked && fetchLikedDetails([loggedInRollNo]); 
    ...
    const handleDeletePost = async (postId) => {
        ...
        try { await deleteDoc(doc(db, "post", postId)); setPosts(c => c.filter(p => p.id !== postId)); interactionState?.post.id === postId && closeInteractionModal(); } catch (err) {console.log(err)
    ...
    const getProfileUrl = useCallback(async (roll) => {
        ...
        let url = null;
        for (const ext of IMAGE_EXTENSIONS) { try { if ((await fetch(`${GITHUB_BASE_URL}${roll}.${ext}`, { method: "HEAD" })).ok) { url = `${GITHUB_BASE_URL}${roll}.${ext}`; break; } } catch (e) {console.log(e)} }
        ...
        const fetchPosts = async () => {
            try {
                ...
                setPosts(list);
                 ...
                 setError("Failed to load posts."); } finally { setLoading(false); }
        };
        fetchPosts();
    }, [getProfileUrl]);
    ...
    const renderPost = (p) => {
        ...
        return (
            <div key={p.id} className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 mx-2 sm:mx-0">
                <div className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                        {hasImg ? (<img src={url} alt="Profile" className="w-11 h-11 rounded-full object-cover border-2 border-blue-500/50" />) : (<div className="w-11 h-11 rounded-full bg-blue-200 flex items-center justify-center text-blue-700 font-extrabold text-lg">{p.name?.charAt(0) || "U"}</div>)}
                        <div>
                            ...
                            <p className="text-xs text-gray-500 flex items-center"><MdOutlineAccessTime className="mr-1" /> {time}</p>
                        </div>
                    </div>
                    <MenuDropdown postId={p.id} showDeleteButton={showDelete} onDeletePost={handleDeletePost} />
                </div>
                {p.text_content && <div className="px-4 pb-2 text-sm text-gray-800 whitespace-pre-wrap">{p.text_content}</div>}
                {p.image_link && <div className="w-full cursor-zoom-in" onClick={() => setZoomedImageLink(p.image_link)}><img src={p.image_link} alt="Post" className="w-full h-auto max-h-[500px] object-cover border-y border-gray-100" /></div>}
                <PostActions post={p} currentRollNo={loggedInRollNo} onLikeToggle={handleLikeToggle} openCommentsModal={() => openModal(p, "comments")} openLikesModal={() => openModal(p, "likes")} />
    ...
    return (
        <div className="max-w-xl mx-auto bg-gray-50 min-h-screen shadow-2xl relative">
            {/* STICKY HEADING SECTION */}
            <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-200 px-4 pb-2 flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-black text-gray-900 flex items-center gap-2">
                        <MdRssFeed className="text-blue-600" /> Notifications
                    </h1>
                    <p className="text-[10px] uppercase tracking-wider text-gray-500 font-bold">Langar Sewa Updates</p>
                </div>
 
            </div>

            <div className="p-0 pt-4 pb-14">
                {loading ? <Loader /> : error ? <p className="text-center text-red-500 p-10">{error}</p> : 
                 posts.length === 0 ? (
                    <div className="flex flex-col items-center justify-center text-center p-6 mt-12">
                        ...
                        <p className="text-sm text-gray-500 mb-6">Posts are automatically removed after 24 hours.</p>
                        <button onClick={() => navigate("/createpost")} className="px-6 py-3 flex items-center gap-2 rounded-full shadow-lg bg-blue-600 text-white font-bold transition-transform hover:scale-105">
                            <MdAddCircle className="text-xl" /> Create New Post
                        </button>
                    </div>
                 ) : (
                    <div className="flex flex-col gap-5 font-[Inter,sans-serif]">{posts.map(renderPost)}</div>
                 )}
            </div>
