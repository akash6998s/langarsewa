...
const Members = () => {
      ...
      if (storedMembers) {
        try {
          ...
          if (!Array.isArray(memberList)) {
            console.warn(
              "Data from 'allMembers' in localStorage is not an array. Resetting."
            );
            memberList = [];
          }
        } catch (parseError) {
  ...
  return (
    <>
      <Topbar />
      <div
        className="pb-24 pt-16 px-4 sm:px-6 lg:px-8 font-[Inter,sans-serif] min-h-screen"
            ...
            <div className="my-6 flex justify-center">
              <div className="relative w-full max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-lg shadow-sm focus:outline-none focus:ring-2"
                  style={{
            ...
            {filteredMembers.length > 0 ? (
              <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8 justify-center">
                {filteredMembers.map((member) => (
                  <div
                    key={member.id}
                    className="rounded-2xl shadow-lg p-6 text-center transition-all duration-300 ease-in-out hover:shadow-2xl hover:-translate-y-2 border transform motion-safe:hover:scale-[1.02] animate-fadeIn"
                    style={{
                  ...
                  >
                    ...
                    <h3
                      className="text-xl font-bold mb-2 leading-tight truncate px-2"
